---
layout: post
title:  "Cybercamp 2018 write-ups: Easy"
...
```
For some time, our client's Wi-Fi network is not working well. They complain that
sometimes the computers do not connect and we suspect that there is a malicious attacker
in the area. We have sent our junior auditor to make a capture on the site, but he has
not been able to determine what is happening and he has sent us the capture in pcap
format. Could you help him? (Answer: flag {MAC address in format: XX:XX:XX:XX:XX:XX of
...
"not working well" may mean that the attacker is kicking the clients to try to get them
to connect to the malicious AP.

To search for these frames, we have to apply the filter `wlan.fc.type_subtype eq 12` to
the captured file:
{% include image.html
	src="/assets/posts/2018-10-12-cybercamp-easy/01.- Filter.jpg"
	title="Filter applied to the capture"
	alt="Wireshark screenshot with the result of applying the filter. The result is a long list of deauth packets sent by the attacker."
%}

Is pretty obvious that the attacker is the one who doesn't stop spamming the deauth
frames: `08:6a:0a:3b:b0:08`.

Thus, the flag is `flag{08:6a:0a:3b:b0:08}`.
...
The description of this challenge states:
```
The personal equipment of a presumed paedophile has been confiscated, but in that
computer it has not been possible to obtain any evidence about the source of the illegal
files that have been downloaded, however, in the folder of evidences there's a video file
that seems suspicious.
```

In our first try[^2], we find an embedded string that's obviously base64 encoded data:
```
$ strings elvis.avi
(...)
...
These are, in fact, two different sets of data, separated by a string of dashes. Once
extracted into their own files and decoded, we have the following:
```
$ file extract_{1,2}
...
Finally, the flag is: `elvissiguevivo`.

-----------------------------------------------------------------------------------------

## 3.- Dude, where is my password?


The description of this challenge states:
```
A friend gives you a memory dump and tells you that you have lost the password, you will
have to manage to find it, the password will be the FLAG sought. Your friend's username
is ThatDude.
```

I couldn't be able to finish this challenge by my own, so I had to read the available
clues, and it really bothers me; because the first two clues were incredibly useless and
the solution only clicked when I read the last one. At that point, I had already lost
like 200 points out of the 300 of the challenge (every clue cost more points than the
previous), and I think I would have classified into the finals if I wouldn't have read
the first two clues... :(

Anyway, let's get to the challenge. We have an `MDMP crash report data`, according to
`file`. After some time searching on the web how to open it or convert it to a Unix core
dump or something[^4], I just gave up and read the first clue:
```
The operative system is a Windows 7 Home Basic x86
```
...

Yeah, that's useless. Let's read the next clue (goodbye, my dear points...):
```
It is a dump of an indispensable process in the security of the system.
```

Hmm...
Nope, still don't know what is this file. Let's hope that the third clue gives some
information:
```
It's a minidump of the process lsass.exe
```

Oooooooooohhhh, damn it.

Suddenly, it clicked: `mimikatz`. Mimikatz is a very versatile tool to bypass Windows
security, extract security information from memory, and millions of other useful things.

Yeah, I should have though about that in the first place...
{% include image.html
	src="/assets/posts/2018-10-12-cybercamp-easy/facepalm.jpg"
	title="No words needed..."
	alt="Facepalm (from https://upload.wikimedia.org/wikipedia/commons/3/3b/Paris_Tuileries_Garden_Facepalm_statue.jpg )"
	style="max-height: 200px"
%}

For this task is easier to use Windows (my main machine is GNU/Linux), so I'll just use a
VM for this. After installing the [mimikatz
binaries](https://github.com/gentilkiwi/mimikatz/releases), we just have to retrieve the
credentials for `ThatDude`:
{% include image.html
	src="/assets/posts/2018-10-12-cybercamp-easy/03.- Mimikatz.jpg"
	title="Result of the execution of mimikatz"
	alt="After loading mi_memoria, the security information of all users is searched. Among the results, there's one for the user TheDude, along with its password"
%}

The flag is: `ImFreeWhyNot98`

-----------------------------------------------------------------------------------------

## 4.- The memory cannot be read

Unfortunately, I lost the description for this challenge; but it said something about a
virus being in the host whose memory dump you're given, and we have to discover who
infected the machine (the flag is the attacker's IP).

So, we have a memory dump and we have to discover who has infected it. As we are going to
work with [volatility](https://github.com/volatilityfoundation/volatility), we first have
to get the profile of the image (the system where the dump was made):
```
...
Now we know that we're dealing with a Windows XP image. The next step is to review the
running processes, to identify some suspicious activity that may be related to the virus:
```
...
Clearly, `hot_pictures.exe` has great chances to be the virus, and we see that its child
process is a command window executing `ping`. Maybe we can check if there's a connection
open with the attacker? Unfortunately not:
```
...
There's no connection open by any of the suspect processes (324, 640 and 1688).
...
And _voilà_, our attack came from **192.168.32.161**.

The flag is: `192.168.21.161`
...
[^2]: Remember, kids: always start by checking the most obvious things. Trust me when I
    say that you don't want to spend a ridiculous amount of time doing complicated things
    when the solution is just as simple as looking for `strings`...

[^3]: We have to import the key first for GPG to work. We could create another keyring,
    or use some of the little hacks that are over the internet; but this way is easier.
    We can remove the key later using `gpg --delete-secret-keys D28DA6DB`.

[^4]: This is really just dumb, as I wouldn't be able to retrieve any information without
    the symbols, anyway... I just didn't really know what to do on this challenge, so I
    tried the first thing that came into mind...
